...
def download_image(lat1: float, lon1: float, lat2: float, lon2: float,
    ...
    img_w, img_h = abs(br_pixel_x - tl_pixel_x), abs(br_pixel_y - tl_pixel_y)

    if img_w <= 0 or img_h <= 0:
        print("Errore: Dimensioni dell'immagine non valide.")
        print(f"tl_pixel_x: {tl_pixel_x}, br_pixel_x: {br_pixel_x}")
        print(f"tl_pixel_y: {tl_pixel_y}, br_pixel_y: {br_pixel_y}")
